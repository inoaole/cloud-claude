...
## Personal OS App Language (design-consultation 2026-07-01)

> Supersedes the 2026-06-28 "폰 콘솔" adaptation. The product pivoted from a phone
> terminal console to a **personal OS** (life-log → growth, calendar, machine access).
> The console is now one module ("Machines"), not the whole app. The base DESIGN.md
> above is Apple **marketing** language — do NOT apply it literally. This section is
> the app's source of truth.

**The one thing to remember:** *"my phone quietly shows me the truth of my day."*
Not flashy — **honest quiet**. Every choice below serves that: mono data chips, `~`
estimates, calm empty states, zero gradients, a single accent. Aesthetic = dark,
restrained, honest. Linear restraint + Warp command-blocks (activity cards) + Blink
key-bar (composer / Machines module). The evening view reads like a **calm daily
journal over a data spine**, never a wall of TUI text.

### Reused tokens (locked, do not drift)
- **Type:** SF Pro Text/Display (reading, UI, reflection, aggregates) + **SF Mono**
  (data: commit shas, repo names, cwd/paths, durations, terminal). This UI-vs-mono
  split *is* the visual system.
- **Accent:** Action Blue `#0066cc` (the ONLY interactive color), focus `#0071e3`,
  on-dark link `#2997ff`.
- **Philosophy:** shadows/gradients restrained (zero decorative gradients), chrome minimal.

### Color (dark-first)
- **Surfaces:** true void `#000` (terminal / data blocks); card surfaces `#1c1c1e`
  (base) / `#2a2a2c` (raised) — iOS dark elevations; hairline `rgba(255,255,255,0.10)`.
- **Text on dark:** primary `#ffffff`, muted `#cccccc`, tertiary `#7a7a7a`.
...
- Ladder unchanged: large-title 34px (collapses to 17px inline on scroll), body 17px,
  footnote 13px, caption 11px. Negative tracking on display sizes (Apple-tight). Weight
  ladder 300/400/600/700 (no 500).
- **Data in SF Mono:** shas, repos, cwd, durations, terminal. Rendered as **data chips**
  inside otherwise-readable SF Pro cards (Warp command-block feel).
- **[RISK 3] Estimate / honesty affordance** — any approximate value (session durations,
  confidence) is rendered `~value` with a leading tilde, muted tone, and a dotted
  underline (`~2h`). The UI must never imply precision it does not have. This is the
...
- Top nav bar + drill-down; large-title → inline on scroll. Min **44px** touch targets.
  Horizontal-scroll **quick-key accessory bar** retained (composer + Machines terminal).
- Mode switches use **segmented controls** (e.g. Machines: terminal / agent).

### Component patterns (new)
- **Activity card** (Warp command-block): SF Pro title + SF Mono data chips (sha / path /
  `~duration`) + muted timestamp. One session or one commit per card.
...
### SAFE vs RISK (why it has its own face)
- **SAFE (category literacy):** dark iOS app, HIG grouped lists + tab bar, SF Pro + SF
  Mono, single Action Blue accent, 44px touch, zero gradients.
- **RISK (memorable):** (1) muted project palette in Growth only, (2) dark Action-Blue
  heat scale, (3) the `~` estimate/honesty affordance as a signature.

### Motion
Minimal-functional. Signature moment: the "reflection saved" quiet check. State
transitions for offline / no-data. Durations from the spec are estimates — never animate
false precision.
